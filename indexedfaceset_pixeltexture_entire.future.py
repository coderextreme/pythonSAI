...
X3D0 = (x3dpsail.X3D().setProfile(x3dpsail.SFString("Interchange")).setVersion(x3dpsail.SFString("3.0"))
      .setHead(x3dpsail.head()
        .addMeta(x3dpsail.meta().setName(x3dpsail.SFString("title")).setContent(x3dpsail.SFString("indexedfaceset_pixeltexture_entire.x3d")))
        ...
        .addMeta(x3dpsail.meta().setName(x3dpsail.SFString("description")).setContent(x3dpsail.SFString("Test browser ability to completely map one PixelTexture onto the surface of an IndexedFaceSet geometry. Four colored squares should map onto each face of the IndexedFaceSet. The PixelTexture consists of red quarter (lower left), green quarter (lower right), white quarter (upper left) and yellow quarter (upper right). PixelTexture should map once onto the surface of the IndexedFaceSet, with the S (horizontal) axis of the texture corresponding to the X axis of the geometry.")))
        .addMeta(x3dpsail.meta().setName(x3dpsail.SFString("identifier")).setContent(x3dpsail.SFString("http://www.web3d.org/x3d/content/examples/ConformanceNist/GeometricProperties/TextureCoordinate/indexedfaceset_pixeltexture_entire.x3d")))
      ...
      .setScene(x3dpsail.Scene()
        ...
        .addChild(x3dpsail.Shape()
          .setAppearance(x3dpsail.Appearance()
            ...
            .setTexture(x3dpsail.PixelTexture().setImage(x3dpsail.SFImage(2,2,4,-16776961,16711935,-1,-65281)).setRepeatS(x3dpsail.SFBool(False)).setRepeatT(x3dpsail.SFBool(False))))
          .setGeometry(x3dpsail.IndexedFaceSet().setCoordIndex(x3dpsail.MFInt32([0,1,3,2,-1,4,5,7,6,-1,6,7,1,0,-1,2,3,5,4,-1,6,0,2,4,-1,1,7,5,3,-1]))
            .setCoord(x3dpsail.Coordinate().setPoint(x3dpsail.MFVec3f([-2,1.5,1,-2,-1.5,1,2,1.5,1,2,-1.5,1,2,1.5,-1,2,-1.5,-1,-2,1.5,-1,-2,-1.5,-1])))))))

X3D0.toFileX3D("./future/./indexedfaceset_pixeltexture_entire_RoundTrip.x3d")
